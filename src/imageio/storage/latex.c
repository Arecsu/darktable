...
static void button_clicked(GtkWidget *widget, dt_imageio_module_storage_t *self)
{
  ...
  if(dt_gui_native_dialog_run(GTK_NATIVE_DIALOG(filechooser)) == GTK_RESPONSE_ACCEPT)
  {
    ...
    gtk_editable_set_text(GTK_EDITABLE(GTK_ENTRY(d->entry)), escaped); // the signal handler will write this to conf
    g_free(dir);
    g_free(composed);
    g_free(escaped);
  }
...
{
  dt_conf_set_string("plugins/imageio/storage/latex/file_directory", gtk_editable_get_text(GTK_EDITABLE(entry)));
}

static void title_changed_callback(GtkEntry *entry, gpointer user_data)
{
  dt_conf_set_string("plugins/imageio/storage/latex/title", gtk_editable_get_text(GTK_EDITABLE(entry)));
}
...
{
  latex_t *d = (latex_t *)self->gui_data;
  gtk_editable_set_text(GTK_EDITABLE(d->entry), dt_confgen_get("plugins/imageio/storage/latex/file_directory", DT_DEFAULT));
  gtk_editable_set_text(GTK_EDITABLE(d->title_entry), dt_confgen_get("plugins/imageio/storage/latex/title", DT_DEFAULT));
  dt_conf_set_string("plugins/imageio/storage/latex/file_directory", gtk_editable_get_text(GTK_EDITABLE(d->entry)));
  dt_conf_set_string("plugins/imageio/storage/latex/title", gtk_editable_get_text(GTK_EDITABLE(d->title_entry)));
}
