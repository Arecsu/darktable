...
static void _property_choice_callback(GtkWidget *item, gpointer user_data)
{
  dt_lib_camera_t *lib = (dt_lib_camera_t *)user_data;
  gtk_editable_set_text(GTK_EDITABLE(GTK_ENTRY(lib->gui.pname)),
                        dt_gui_menu_item_get_label(item));
}
...
static void _show_property_popupmenu_clicked(GtkWidget *widget, gpointer user_data)
{
  ...
  // the menu is one-shot (self-destructs on close), so build a fresh one
  // each time the popup is requested; it needs a live camera connection
  GtkWidget *menu = NULL;
  dt_camctl_camera_build_property_menu(darktable.camctl, NULL, &menu,
                                       G_CALLBACK(_property_choice_callback), lib);
  if(menu)
    dt_gui_menu_popup(menu, widget);
}
