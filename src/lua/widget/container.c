...
static int container_reset(lua_State* L)
{
  ...
  for(GtkWidget *cur_widget = gtk_widget_get_first_child(container->widget);
      cur_widget;
      cur_widget = gtk_widget_get_next_sibling(cur_widget))
  {
...
static int container_numindex(lua_State*L)
{
  lua_container container;
  luaA_to(L,lua_container,&container,1);
  GtkWidget *first = gtk_widget_get_first_child(container->widget);
  int length = 0;
  ...
  if(lua_gettop(L) >2) {
    if(!lua_isnil(L,3) &&  index == length) {
      ...
      // GTK4 has no generic container add: dispatch on the container type
      if(GTK_IS_BOX(container->widget))
        gtk_box_append(GTK_BOX(container->widget),widget->widget);
      else if(GTK_IS_STACK(container->widget))
        gtk_stack_add_named(GTK_STACK(container->widget),widget->widget,NULL);
      else
        gtk_widget_set_parent(widget->widget,container->widget);
      ...
      GtkWidget *searched_widget = first;
      for(int i = 0; i < index; i++) searched_widget = gtk_widget_get_next_sibling(searched_widget);
      gtk_widget_unparent(searched_widget);
    } else {
...
{
  dt_lua_init_widget_type(L,&container_type,lua_container,GTK_TYPE_WIDGET);
